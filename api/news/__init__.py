...
as JSON.  Results are cached in a module-level variable for 5 minutes so cold
starts on a busy day don't hammer the upstream feeds.
"""
...
# ---------------------------------------------------------------------------
# Module-level cache  { "ts": float, "items": list }
# ---------------------------------------------------------------------------
_cache: dict = {}
...
def _get_items() -> list:
    """Return cached items, refreshing when the TTL has expired."""
    now = time.time()
...
def _fetch_all() -> list:
    """Fetch all feeds in parallel and return the top MAX_ITEMS by date."""
    results = []
    ...
    # Sort newest-first, take top MAX_ITEMS
